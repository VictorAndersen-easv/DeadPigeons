    ...
    AppOptions appOptions) : IAuthService
{
    public async Task<JwtClaims> VerifyAndDecodeToken(string token)
    {
        ...
        _ = ctx.Players.FirstOrDefault(u => u.Id == jwtClaims.Id)
            ?? throw new ValidationException("Authentication is valid, but player is not found!");
    ...
    public async Task<JwtResponse> Login(LoginRequestDto dto)
    {
        var player = ctx.Players.FirstOrDefault(u => u.Email == dto.Email)
                   ?? throw new ValidationException("player is not found!");
        var passwordsMatch = player.Passwordhash ==
                             SHA512.HashData(
                                     Encoding.UTF8.GetBytes(dto.Password + player.Salt))
                                 .Aggregate("", (current, b) => current + b.ToString("x2"));
        if (!passwordsMatch)
            throw new ValidationException("Password is incorrect!");

        var token = CreateJwt(player);
    ...
    public async Task<JwtResponse> Register(RegisterRequestDto dto)
    {
        ...
        var isEmailTaken = ctx.Players.Any(u => u.Email == dto.Email);
        if (isEmailTaken)
    ...
    private string CreateJwt(Player player)
    {
        return CreateJwtBuilder()
            .AddClaim(nameof(Player.Id), player.Id)
            .Encode();
    }
}